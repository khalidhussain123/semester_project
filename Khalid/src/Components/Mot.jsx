    ...
    <div className='bg-gradient-to-b from-black via-gray-900 to-black py-12' id='trainer'>
        ...
        <div className='text-center mb-10 px-4 '>
            <h2 className='text-3xl md:text-5xl font-extrabold tracking-tight text-gray-800'>
                <span className='bg-clip-text text-transparent bg-gradient-to-r from-blue-400 to-purple-600'>Meet Our Trainers</span>
            </h2>
            <p className='text:sm sm:text-lg md:text-lg text-gray-400  mt-4 max-w-2xl mx-auto'>Lorem ipsum dolor sit, amet consectetur adipisicing elit. Consectetur dolores, maiores officiis numquam itaque rerum, odio deleniti at recusandae odit ipsam! Temporibus, deleniti?</p>
        </div>
        {/* image grids  */}
            <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 px-4 md:px-10 lg:px-20'>
                {trainers.map((imgsrc,index)=>(
                    <div key={index} className='group relative overflow-hidden rounded-xl shadow-lg bg-gradient-to-br from-gray-800 to-gray-900 transition-transform transform hover:scale-105 hover:shadow-2xl'>
                        {/* lazy loading  */}
                        <img src={imgsrc} alt={`Trainers ${index + 1}`} className='w-full aspect-{3/2} object-cover transition-transform duration-500 ease-out group-hover:scale-105' />
                        {/* overlay content   */}
                        <div className='absolute inset-0 bg-gradient-to-t from-black/80 via-black/60 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 ease-in-out flex flex-col justify-end p-4'>
                            ...
                            <p className='text-xs md:text-sm text-gray-300'>Certified Fitness & Wellness Couch</p>
                        </div>
                    </div>
                ))}
            </div>
